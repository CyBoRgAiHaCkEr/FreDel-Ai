...
# --- 2. THE SAFE IMAGE PROXY ---
def get_base64_image(prompt):
    seed = np.random.randint(0, 999999)
    url = f"https://pollinations.ai/p/{prompt.replace(' ', '%20')}?width=1024&height=1024&seed={seed}&nologo=true"
    try:
        # Python fetches the image data (server-side)
        response = requests.get(url, timeout=20)
        if response.status_code == 200:
            # Convert to Base64 to bypass browser security
...
if prompt := st.chat_input("Command FreDèlAi..."):
    ...
    with st.chat_message("assistant"):
        # IMAGE GENERATION via HTML Proxy
        if any(x in prompt.lower() for x in ["draw", "image", "paint", "generate"]):
            with st.spinner("🎨 Creating..."):
                img_html = get_base64_image(prompt)
