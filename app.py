...
with st.sidebar:
    ...
    # NEW: French Mode Toggle
    is_fr = st.toggle("🇫🇷 Mode Français", value=False, help="Forces AI to respond in French")
    
    st.divider()
    st.write(f"🧠 Memory Slots: {len(st.session_state.patterns)} / 10000")
    ...
    if st.button("⚡ Process & Remember"):
        # Detect language based on toggle
...
if prompt := st.chat_input("Command or Chat..."):
    # 1. Add User Input to Display
    st.session_state.messages.append({"role": "user", "content": prompt})
    with st.chat_message("user"): st.markdown(prompt)

    # --- STEP 1: COMMAND ASSIGNMENT (X is Y) ---
    if " is " in prompt.lower() and len(prompt.split()) < 30:
        ...
        response = f"✅ Learned! '{key.strip()}' memorized. ({len(st.session_state.patterns)}/3500)"
        with st.chat_message("assistant"): st.markdown(response)
        st.session_state.messages.append({"role": "assistant", "content": response})

    # --- STEP 2: HARD SHORTCUT BYPASS (e.g. 'taii') ---
    elif prompt.lower().strip() in st.session_state.patterns:
        # PURE RECALL: No AI, No Bonjour.
    ...
    # --- STEP 3: OPENAI GPT-OSS-120B ENGINE ---
    else:
        with st.chat_message("assistant"):
            try:
                client = Groq(api_key=st.secrets["GROQ_API_KEY"])
                
                # Dynamic System Message based on Language Toggle
                sys_msg = (
                    f"You are FreDèlAi, the AI assistant for DELU, the expert French teacher from Mumbai. "
                    f"Core Knowledge: {st.session_state.brain_memory[-2500:]}. "
                    "Refer to Delu as the lead educator. Be structured and pattern-oriented."
                )
                if is_fr:
                    sys_msg += " RESPOND ENTIRELY IN FRENCH. Use professional and academic tone."
                
                # Model updated to openai/gpt-oss-120b
                r = client.chat.completions.create(
                    model="openai/gpt-oss-120b",
                    messages=[{"role":"system","content":sys_msg}] + st.session_state.messages[-12:]
                )
                ans = r.choices[0].message.content
                st.markdown(ans)
                st.session_state.messages.append({"role": "assistant", "content": ans})
            except Exception as e:
                st.error(f"Engine Error: {e}. Please ensure Groq supports this specific model ID.")

# Chat Cleanup (Last 50 messages)
